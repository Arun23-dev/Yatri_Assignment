...
export class ChargingSessionsService {
  ...
  async getChargingSessionById(id: string) {
  ...
  async startChargingSession(customerId: string, data: { bikeId: string; amount: number }) {
    // Check if customer already has an active session for this bike
    ...
    if (existingActiveSession) {
      throw new BadRequestException('Customer already has an active charging session for this bike');
    }
    ...
    if (bike.status !== 'AVAILABLE') {
      throw new BadRequestException('Bike is not available for charging');
    }
  ...
  async endChargingSession(customerId: string, sessionId: string, endTime?: Date) {
    ...
    if (session.customerId !== customerId) {
      throw new BadRequestException('This charging session does not belong to you');
    }

    // Check if the session is active
    if (session.status !== 'ACTIVE') {
      throw new BadRequestException(`Charging session is not active. Current status: ${session.status}`);
    }

    // Check if session already has an end time
    if (session.endTime) {
      throw new BadRequestException('Charging session has already ended');
    }
  ...
  async cancelChargingSession(customerId: string, sessionId: string) {
    // First check if the session exists
    const session = await this.prisma.chargingSession.findUnique({
      where: { id: sessionId },
      include: {
  ...
  async getActiveSessionsByCustomer(customerId: string) {
  ...
  async getMySessions(customerId: string, filters: {
    bikeId?: string;
    page?: number;
    limit?: number;
  ...
  async getSessionDetails(customerId: string, sessionId: string) {
  ...
  async getCurrentActiveSession(customerId: string) {
    const session = await this.prisma.chargingSession.findFirst({
      where: {
        customerId,
        status: 'ACTIVE',
    ...
    if (!session) {
      throw new NotFoundException('No active charging session found');
    }

    return session;
  }

  async getCustomerAnalytics(customerId: string, dateRangeStart?: string, dateRangeEnd?: string) {
    ...
    // Get sessions per day with detailed breakdown
    ...
    // Get recent sessions with timing details
    ...
    const enhancedResponse = {
      ...
      // Recent sessions with detailed timing
      recentSessions,
  ...
  // Helper method to calculate hourly distribution
  private calculateHourlyDistribution(sessions: any[]): { hour: number; count: number; percentage: number }[] {
  ...
  private calculateDailyStats(sessionsPerDay: any[]): {
    ...
    const mostActiveDay = sessionsPerDay.reduce((max, day) => 
      day._count.id > max._count.id ? day : max
    );
  ...
  async adminEndSession(sessionId: string, endTime?: Date) {
    const session = await this.prisma.chargingSession.findUnique({
      where: { id: sessionId },
      include: {
        customer: {
  ...
  async getAdminAnalyticsSummary(dateRangeStart?: string, dateRangeEnd?: string) {
    const whereConditions: any = {};
    ...
    return {
      ...
      totalEnergyDelivered: totalRevenue._sum.amount || 0, // kWh equivalent
  ...
  async getChargingSessionStats(period?: string, startDate?: string, endDate?: string) {
    let dateFilter: any = {};
    ...
    if (startDate && endDate) {
      ...
      switch (period) {
        ...
        case 'year':
          ...
          break;
        default:
          start = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000); // Default to last 30 days
      }
  ...
  async getRevenueAnalytics(period: string = 'month', groupBy: string = 'day') {
    const now = new Date();
    let start: Date;
  ...
  async getChargingSessionsByCustomer(customerId: string, page: number = 1, limit: number = 10) {
    const skip = (page - 1) * limit;
  ...
  async getChargingSessionsByBike(bikeId: string, page: number = 1, limit: number = 10) {
    const skip = (page - 1) * limit;
  ...
  async getChargingSessionReports(filters: {
    ...
    // Add transaction data (simulated)
